...
public class  DirectRabbitConfig {
    ...
    DirectExchange LogDirectExchange(){
        ...
        return new DirectExchange(MqEnums.SYS_LOG.getExchange(),true,false);
    }
    //绑定交换机和队列
    @Bean
    Binding BindingDirect(){
        //bind队列to交换机中with路由key（routing key）
        return BindingBuilder.bind(LogDirectQueue()).to(LogDirectExchange()).with(MqEnums.SYS_LOG.getRoutKey());
    }
}
